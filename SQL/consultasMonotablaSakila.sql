...
# EJ39 ¿HAY ALGUNA PELICULA PARA LA QUE NO TENGAMOS NINGUNA COPIA?
SELECT *
FROM FILM
WHERE FILM_ID
NOT IN (SELECT FILM_ID FROM INVENTORY);

SELECT *	# ESTA FORMA MEJOR
FROM FILM
LEFT JOIN INVENTORY INV ON FILM.FILM_ID = INV.FILM_ID
WHERE INV.INVENTORY_ID IS NULL;

# EJ40 MUESTRA UNA LISTA DE LAS PELICULAS QUE NOS FALTAN EN LA TIENDA 2.
SELECT * FROM INVENTORY;

SELECT *
FROM FILM
LEFT JOIN INVENTORY INV ON FILM.FILM_ID = INV.FILM_ID AND INV.STORE_ID = 2
WHERE INV.INVENTORY_ID IS NULL;

# EJ41 ¿CUAL ES EL ACTOR MAS FAMOSO? (AQUEL CUYAS PELICULAS SE HAYAN ALQUILADO MAS VECES)
SELECT 
	ACTOR.FIRST_NAME,
    ACTOR.LAST_NAME,
    COUNT(*) AS PELICULAS_ALQUILADAS
FROM RENTAL
JOIN INVENTORY AS INV ON RENTAL.INVENTORY_ID = INV.INVENTORY_ID
...
# EJ42 ¿CUAL ES LA PELICULA MAS ALQUILADA DE LA TIENDA 2?
SELECT 
	FILM.TITLE,
    COUNT(*) AS PELICULAS_ALQUILADAS
FROM RENTAL
...
# EJ43 ¿DESDE QUE PAIS NOS HAN ALQUILADO MAS PELICULAS? (CUSTOMER)
SELECT 
	CO.COUNTRY,
    COUNT(*) AS ALQUILERES
FROM RENTAL 
JOIN CUSTOMER AS CUS ON RENTAL.CUSTOMER_ID = CUS.CUSTOMER_ID
JOIN ADDRESS AS ADR ON CUS.ADDRESS_ID = ADR.ADDRESS_ID
JOIN CITY AS CI ON CI.CITY_ID = ADR.CITY_ID
JOIN COUNTRY AS CO ON CO.COUNTRY_ID = CI.COUNTRY_ID
GROUP BY CO.COUNTRY_ID
ORDER BY ALQUILERES DESC
LIMIT 1;

# EJ44 MUESTRA LA DIRECCION DE CADA TIENDA Y LA CANTIDAD DE USUARIOS INACTIVOS QUE HAY EN CADA UNA
SELECT 
	AD.ADDRESS,
    COUNT(*)
